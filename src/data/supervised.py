...
def induced_graphs(data, smallest_size=10, largest_size=30):
    ...
    for index in range(data.x.size(0)):
        ...
        while len(subset) < smallest_size and current_hop < 5:
            current_hop += 1
            subset, _, _, _ = k_hop_subgraph(node_idx=index, num_hops=current_hop, edge_index=data.edge_index)
...
def get_supervised_data(dataset, ratios, seed=777, cache_dir='storage/.cache', few_shot=1, node_feature_dim=0):
    import os
    ...
    results = [{'train': train_set, 'val': val_set, 'test': test_set}, num_classes]
