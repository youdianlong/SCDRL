...
from torch_geometric.datasets import Amazon, Planetoid, WebKB, WikipediaNetwork
...
def iterate_datasets(data_names, cache_dir='storage/.cache'):
...
def iterate_dataset_feature_tokens(data_names, cache_dir='storage/.cache'):
    yield from iterate_datasets(data_names, cache_dir=cache_dir)


def preprocess(data, node_feature_dim=0):
...
def gen_ran_output(data, simgrace, reconstruct=0.0):
    vice_model = deepcopy(simgrace)

    for (vice_name, vice_model_param), (_, param) in zip(vice_model.named_parameters(), simgrace.named_parameters()):
    ...
    if reconstruct == 0.0:
        return vice_model.forward_cl(data)
